...
best_churn["nr_of_transactions"] = group_by_customer.size()
best_churn["amount_spent"] = group_by_customer.sum()
best_churn.drop(["nr_of_transactions","trans_date"], axis="columns", inplace=True)


## 6. Ranking Customers ##

best_churn["scaled_tran"]=(best_churn["nr_of_transactions"]-min(best_churn["nr_of_transactions"]))/(max(best_churn["nr_of_transactions"])-min(best_churn["nr_of_transactions"]))
best_churn["scaled_amount"]=(best_churn["amount_spent"]-min(best_churn["amount_spent"]))/(max(best_churn["amount_spent"])-min(best_churn["amount_spent"]))
