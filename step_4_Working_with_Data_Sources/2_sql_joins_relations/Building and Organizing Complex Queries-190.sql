...
WITH 
    firstname AS
        (
        SELECT first_name||" "||last_name customer_name, customer_id FROM customer WHERE country="India"
        ),
    totall AS
        (
        SELECT sum(total) total_purchases, customer_id from invoice GROUP by customer_id
        )
SELECT customer_name,total_purchases  from firstname f
INNER JOIN totall t
